...
<body class="min-h-screen bg-[#111111] text-gray-100 font-sans">
...
<div class="max-w-7xl mx-auto p-6 mt-20">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
        ...
        <div>
            <img src="{{ asset('storage/' . ($fotos[0] ?? 'sem-imagem.png')) }}"
                 class="w-full h-[480px] object-cover rounded-md border border-gray-800 shadow main-image">
            ...
            <div class="flex gap-3 mt-4">
                @foreach(array_slice($fotos, 1) as $foto)
                    <img src="{{ asset('storage/' . $foto) }}"
                         class="w-20 h-20 rounded-md border border-gray-700 cursor-pointer hover:opacity-80"
    ...
    @endif
        ...
        <div class="flex flex-col gap-6">
            <h1 class="text-4xl font-bold tracking-tight">{{ $produto->nome }}</h1>
            ...
            <div>
                ...
                <span class="text-sm text-green-500">22% OFF</span>
            </div>
                ...
                <div class="flex gap-3">
                    @foreach($coresComPrincipal as $img)
                        <img src="{{ asset('storage/' . $img) }}"
                             class="w-16 h-16 rounded-md border border-gray-700 cursor-pointer hover:opacity-80 transition"
                             onclick="document.querySelector('.main-image').src=this.src">
                    @endforeach
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2 mt-2">
                @foreach($tamanhos as $tamanho)
                    <button type="button" 
                            class="tamanho-btn-produto px-4 py-2 border border-gray-600 rounded-md hover:bg-gray-800 transition"
            ...
            <form action="{{ route('carrinho.adicionar', $produto->id_produtos) }}" method="POST" id="form-carrinho-principal">
                ...
                <button type="submit" class="w-96 bg-white text-black font-bold py-2 px-4 rounded-md text-lg hover:bg-gray-200 transition">
                    Adicionar ao carrinho
                </button>
                <p id="erroTamanhoPrincipal" class="text-red-500 mt-2 hidden">Por favor, selecione um tamanho antes.</p>
            </form>
        </div>
    </div>
    ...
    <div class="mt-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            @foreach($produtos as $prod)
                ...
                <div class="bg-[#1a1a1a] rounded-md border border-gray-800 shadow hover:shadow-lg transition p-4 flex flex-col">
                    ...
                    <div class="flex gap-2 mb-3">
                        @foreach($estoqueRec as $img)
                            <img src="{{ asset('storage/' . $img) }}" 
                                 class="w-10 h-10 rounded border border-gray-700 cursor-pointer hover:opacity-80"
                    ...
                    <div class="flex flex-wrap gap-2 mb-3">
                        @foreach($tamanhosRec as $tam)
                            <button type="button" class="tamanho-btn-rec px-3 py-1 border border-gray-600 rounded-md text-sm hover:bg-gray-800 transition" data-tamanho="{{ $tam }}">
                                {{ $tam }}
                            </button>
                        @endforeach
                    </div>
                    ...
                    <form action="{{ route('carrinho.adicionar', $prod->id_produtos) }}" method="POST" class="form-carrinho-rec mt-auto">
                        ...
                        <button type="submit" class="w-full bg-white text-black font-bold py-2 rounded-md hover:bg-gray-200 transition">
                            Adicionar
                        </button>
...
<script>
    ...
    tamanhoBtnsPrincipal.forEach(btn=>{
        btn.addEventListener('click', ()=>{
            tamanhoBtnsPrincipal.forEach(b=>b.classList.remove('bg-gray-800','border-white'));
            btn.classList.add('bg-gray-800','border-white');
    ...
    document.querySelectorAll('.form-carrinho-rec').forEach(form=>{
        ...
        tamanhoBtns.forEach(btn=>{
            btn.addEventListener('click', ()=>{
                tamanhoBtns.forEach(b=>b.classList.remove('bg-gray-800','border-white'));
                btn.classList.add('bg-gray-800','border-white');
