<div class="relative w-full">
    {{-- Botão de lista de desejos (fora do link, sobre o card) --}}
    <form action="{{ route('lista-desejos.store', $produto->id_produtos) }}" method="POST" 
          class="absolute top-2 right-2 z-50">
        @csrf
        <button type="submit"
            onclick="event.stopPropagation();"
            class="pointer-events-auto w-10 h-10 flex items-center justify-center bg-[#111] border border-[#d5891b]/50 rounded-lg hover:bg-[#222] transition">
    ...
    <a href="{{ route('produtos.detalhes', $produto->id_produtos) }}" class="block w-full h-full relative z-10">
        <div class="bg-[#111]/50 border border-[#222] rounded-xl shadow-lg p-6 min-h-[580px] cursor-pointer hover:border-[#D5891B]/20 transition-all duration-200 flex flex-col">
            ...
            @endphp
            <div class="w-full mb-4 flex-shrink-0">
                <img
