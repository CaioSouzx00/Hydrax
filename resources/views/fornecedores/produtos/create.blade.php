...
<body class="min-h-screen overflow-x-hidden relative">
  ...
  <main class="flex items-center justify-center min-h-screen px-4">
    <div class="w-full max-w-5xl bg-black/30 border border-purple-700/30 rounded-2xl shadow-[0_0_30px_rgba(139,92,246,0.1)] p-10">
      ...
      <form action="{{ route('fornecedores.produtos.store') }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6" enctype="multipart/form-data">
        @csrf

        <input type="text" name="nome" placeholder="Nome" class="input-style" required />
        <input type="text" name="descricao" placeholder="Descrição" class="input-style" />
        <input type="number" name="preco" placeholder="Preço" step="0.01" class="input-style" required />
        <input type="text" name="estoque_imagem" placeholder="Estoque Imagem" class="input-style" />
  ...
  <style>
    .input-style {
      @apply w-full px-4 py-3 bg-gray-900/40 text-white/70 placeholder-white/30 hover:text-white focus:text-white border border-indigo-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-indigo-900/40 transition;
    }
  </style>
</body>
</html>
