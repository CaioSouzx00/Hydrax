<div class="bg-gradient-to-br from-[#211828] via-[#0b282a] to-[#17110d] border border-[#d5891b]/50 rounded-xl mb-12 shadow p-8 max-w-7xl mx-auto text-white">
    ...
    <div class="mb-4 relative max-w-sm">
        <input type="text" id="produto-search" placeholder="Pesquisar produtos..."
            class="w-full pl-10 pr-4 py-2 rounded-lg bg-gray-800/50 text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#d5891b] transition">
        <div class="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#d5891b]">
        ...
        <table class="min-w-full text-left text-sm" id="produtos-table">
            <thead class="bg-[#7f3a0e] text-white">
                <tr>
                    ...
                    <th class="px-6 py-3">Estoque (Qtd)</th>
                    <th class="px-6 py-3">Fotos</th>
                    <th class="px-6 py-3">Estoque (Imagens)</th>
                    <th class="px-6 py-3 pl-10">Ações</th>
                </tr>
            </thead>
                ...
                <tr class="hover:bg-[#0b282a]/70 transition">
                    ...
                    <td class="px-6 py-4">
                        <span class="text-gray-300">
                            {{ \App\Models\ProdutoEstoque::where('produto_id', $produto->id_produtos)->sum('quantidade') }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        @php
                            ...
                            if (!is_array($fotosProduto)) {
                                $fotosProduto = json_decode((string)$fotosProduto, true);
                                if (is_string($fotosProduto)) {
                                    $fotosProduto = json_decode($fotosProduto, true);
                                }
                            }
                        @endphp

                        @if(!empty($fotosProduto) && is_array($fotosProduto))
...
<td class="px-6 py-4">
    ...
    @endif
                    ...
                    <td class="px-6 py-4 flex flex-col pl-10 gap-1">
                        ...
                        <a href="#"
                            data-url="{{ route('fornecedores.produtos.estoque.edit', $produto->id_produtos) }}"
