<div class="bg-gradient-to-br from-[#211828] via-[#0b282a] to-[#17110d] border border-[#d5891b]/50 rounded-xl shadow-lg p-8 max-w-6xl mx-auto mb-12 text-white">
  ...
  <input name="cor" value="{{ old('cor', $produto->cor) }}" 
    ...
    <!-- Marca -->
    <div>
      <label class="block text-sm font-medium text-[#d5891b]">Marca</label>
      <select name="marca_id" class="w-full h-10 bg-[#17110d] border border-[#d5891b]/40 rounded px-2 text-white/70 hover:text-white">
        <option value="">Selecione a marca</option>
        @foreach(($marcas ?? collect()) as $marca)
          <option value="{{ $marca->id }}" @selected((int)old('marca_id', $produto->marca_id) === (int)$marca->id)>{{ $marca->nome }}</option>
        @endforeach
      </select>
    </div>
