...
<div class="container mx-auto p-6 space-y-6">
    ...
    <div class="mt-4 flex flex-col sm:flex-row gap-3">
        @if(in_array($pedido->status, ['finalizado', 'criado', 'aguardando_pagamento']))
            <form method="POST" action="{{ route('pedidos.cancelar', $pedido->id) }}">
                @csrf
                <button type="submit" class="px-5 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-white font-semibold transition">
                    Cancelar pedido
                </button>
            </form>
        @endif

        <form method="POST" action="{{ route('pedidos.recomprar', $pedido->id) }}">
            @csrf
            <button type="submit" class="px-5 py-2 bg-[#14ba88] hover:bg-[#117c66] rounded-lg text-black font-semibold transition">
                Recomprar
            </button>
        </form>
    </div>
    ...
    @if(!empty($pedido->codigo_rastreio) || !empty($pedido->url_rastreio))
        <div class="mt-4 text-sm text-gray-300 bg-[#0b282a] rounded-2xl p-4 border border-[#14ba88]/20 shadow-inner">
            <p class="font-semibold text-[#14ba88] mb-2">Rastreamento:</p>
            @if(!empty($pedido->codigo_rastreio))
                <p>Código: <span class="text-white font-semibold">{{ $pedido->codigo_rastreio }}</span></p>
            @endif
            @if(!empty($pedido->url_rastreio))
                <p>
                    <a class="underline text-[#e29b37]" href="{{ $pedido->url_rastreio }}" target="_blank" rel="noopener noreferrer">Acompanhar entrega</a>
                </p>
            @endif
        </div>
    @endif
...
@php
    $etapas = [
        ...
        ['nome' => 'Pago', 'icone' => '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-2.5 0-4.5 1.12-4.5 2.5S9.5 13 12 13s4.5 1.12 4.5 2.5S14.5 18 12 18m0-10V6m0 12v-2" /></svg>'],
        ['nome' => 'Enviado', 'icone' => '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 13h2l2 5h11l2-5h2" /><circle cx="7.5" cy="18.5" r="1.5" /><circle cx="16.5" cy="18.5" r="1.5" /></svg>'],
...
@foreach($pedido->itens as $item)
    @php
        ...
        $fotosArr = is_array($item->produto->fotos ?? null) ? $item->produto->fotos : json_decode($item->produto->fotos ?? '[]', true);
        $foto = $fotosArr[0] ?? null;
