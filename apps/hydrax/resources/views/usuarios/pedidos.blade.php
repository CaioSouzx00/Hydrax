                   ...
                   class="block bg-[#1e1e2a]/50 border border-[#14ba88]/20 rounded-2xl p-6 hover:border-[#14ba88]/50 transition flex flex-col gap-4">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="px-3 py-1 rounded-full text-sm font-semibold 
                                     {{ in_array($pedido->status, ['entregue'], true) ? 'bg-[#14ba88] text-black' : (in_array($pedido->status, ['cancelado','reembolsado'], true) ? 'bg-red-600 text-white' : 'bg-[#e29b37] text-black') }}">
                    ...
                    @foreach($pedido->itens as $item)
                        @php 
                            ...
                            $fotos = is_array($item->produto->fotos ?? null) ? $item->produto->fotos : json_decode($item->produto->fotos ?? '[]', true);
                            $foto = $fotos[0] ?? null;
                        @endphp
