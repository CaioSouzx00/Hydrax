...
<a href="{{ route('usuarios.pedidos') }}" class="relative hover:text-gray-300 transition ml-4">
    ...
    @php
        $pedidosCount = Auth::guard('usuarios')->check()
            ? \App\Models\Pedido::where('usuario_id', Auth::guard('usuarios')->user()->id_usuarios)->count()
            : 0;
    @endphp
...
<form id="sidebar-filtros" method="GET" action="{{ route('dashboard') }}" 
    ...
    <!-- Marca -->
    <h3 class="font-semibold mt-5 mb-2 text-[#d5891b]">Marca</h3>
    <select name="marca"
            class="w-full p-2 bg-black/30 text-white border border-[#d5891b]/50 rounded-lg backdrop-blur-sm focus:ring-2 focus:ring-[#d5891b] focus:outline-none">
        <option value="">Todas</option>
        @foreach(($marcas ?? collect()) as $marca)
            <option value="{{ $marca->id }}" {{ (string)request('marca') === (string)$marca->id ? 'selected' : '' }}>
                {{ $marca->nome }}
            </option>
        @endforeach
    </select>
