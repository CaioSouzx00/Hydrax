...
<div class="mt-4">
    ...
    <div class="flex gap-3">
        @foreach($variantes as $variante)
            @php
                $fotosVariante = $variante->fotos;
                if (!is_array($fotosVariante)) {
                    $fotosVariante = json_decode((string) $fotosVariante, true);
                    if (is_string($fotosVariante)) {
                        $fotosVariante = json_decode($fotosVariante, true);
                    }
                }
                $fotoVariantePrincipal = (is_array($fotosVariante) && !empty($fotosVariante)) ? $fotosVariante[0] : null;
            @endphp
            <img 
                src="{{ $fotoVariantePrincipal ? asset('storage/' . $fotoVariantePrincipal) : '' }}" 
                alt="{{ $variante->cor }}" 
