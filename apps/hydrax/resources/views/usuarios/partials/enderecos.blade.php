          ...
          <tr class="hover:bg-[#2a2a2a]/50 transition">
            ...
            <td class="px-4 py-3 border-b border-[#2c2c2c]">
              ...
              @if($endereco->is_principal)
                <span class="ml-2 text-xs bg-[#14ba88] text-black px-2 py-1 rounded">Principal</span>
              @endif
            </td>
            <td class="px-4 py-3 border-b border-[#2c2c2c]">
              <div class="flex items-center gap-4">
                @if(!$endereco->is_principal)
                  <form method="POST" action="{{ route('usuarios.enderecos.principal', $endereco->id_endereco) }}" class="inline">
                    @csrf
                    <button type="submit" class="text-[#14ba88] hover:text-[#1ce0a5] transition flex items-center gap-1">
                      Definir Principal
                    </button>
                  </form>
                @endif
