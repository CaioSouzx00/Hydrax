...
class ProdutoFornecedorController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('fornecedores.produtos.partials.create', compact('marcas'));
    }
    ...
    public function edit($id)
    {
        ...
        $marcas = Marca::where('ativo', true)->orderBy('nome')->get();
        return view('fornecedores.produtos.partials.edit', compact('produto', 'marcas'));
    }
...
public function autoImport(Request $request)
{
    ...
    $extractPath = storage_path('app/auto_import/' . uniqid());
    $zip->extractTo($extractPath);
    $zip->close();

    // Percorre tudo recursivamente
    $pastas = File::directories($extractPath);

    foreach ($pastas as $marcaPath) {
        $modelos = File::directories($marcaPath);

        foreach ($modelos as $modeloPath) {
            $cores = File::directories($modeloPath);

            foreach ($cores as $corPath) {

                $fotos = [];
                foreach (File::files($corPath) as $img) {
                    $fotos[] = Storage::putFile('produtos', $img);
                }

                if (count($fotos) === 0) continue;

                $nomeProduto = basename($modeloPath) . ' - ' . basename($corPath);

                $this->produtoService->criarProduto([
                    'nome' => $nomeProduto,
                    'preco' => rand(600, 950),
                    'descricao' => 'Produto importado automaticamente pelo sistema',
                    'tamanhos_disponiveis' => json_encode([38,39,40,41,42,43]),
                    'fotos' => json_encode($fotos),
                    'ativo' => true
                ], $fornecedor->id_fornecedores);
            }
        }
    }
    ...
    return back()->with('success', 'Importação automática concluída 😴🔥');
}
