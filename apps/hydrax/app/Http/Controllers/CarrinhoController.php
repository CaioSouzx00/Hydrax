...
class CarrinhoController extends Controller
{
    ...
    public function meusPedidos()
    {
        ...
        $pedidos = Pedido::where('usuario_id', $usuario->id_usuarios)
    ...
    public function detalhePedido($pedidoId)
    {
        ...
        $pedido = Pedido::where('usuario_id', $usuario->id_usuarios)
            ->with('itens.produto', 'endereco')
            ->findOrFail($pedidoId);
    ...
    public function cancelarPedido($pedidoId)
    {
        ...
        $pedido = Pedido::where('usuario_id', $usuario->id_usuarios)
            ->with(['itens', 'usuario', 'itens.produto'])
            ->findOrFail($pedidoId);

        $statusCancelavel = in_array($pedido->status, ['finalizado', 'criado', 'aguardando_pagamento'], true);
        if (!$statusCancelavel) {
    ...
    public function recomprarPedido($pedidoId)
    {
        $usuario = Auth::guard('usuarios')->user();
        ...
        return redirect()->route('carrinho.ver')->with('success', 'Itens adicionados ao carrinho.');
    }
