...
public function pedidos(Request $request)
{
    ...
    $query = Pedido::with(['usuario', 'itens.produto']);
    ...
    return view('admin.pedidos', compact('pedidos', 'status', 'busca'));
}

public function atualizarPedido(Request $request, $pedidoId)
{
    $data = $request->validate([
        'status' => 'required|string',
        'codigo_rastreio' => 'nullable|string|max:255',
        'url_rastreio' => 'nullable|string|max:255',
    ]);

    $pedido = Pedido::with(['usuario', 'itens.produto'])->findOrFail($pedidoId);

    $pedido->status = $data['status'];
    $pedido->codigo_rastreio = $data['codigo_rastreio'] ?? null;
    $pedido->url_rastreio = $data['url_rastreio'] ?? null;
    $pedido->save();
